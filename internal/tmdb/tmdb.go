...
func (c *Client) search(ctx context.Context, query string, year int) ([]searchResult, error) {
	...
	endpoint := c.baseURL + "/search/movie?" + params.Encode()

	req, err := http.NewRequestWithContext(ctx, http.MethodGet, endpoint, nil)
	if err != nil {
		return nil, fmt.Errorf("tmdb search %q: build request: %w", query, err)
	}
	req.Header.Set("Accept", "application/json")

	resp, err := c.http.Do(req)
	if err != nil {
		return nil, fmt.Errorf("tmdb search %q: %w", query, scrubURL(err))
	}
	...
	case http.StatusUnauthorized:
		return nil, fmt.Errorf("tmdb search %q: %w: %s", query, ErrUnauthorized, statusMessage(resp.Body))
	default:
		return nil, fmt.Errorf("tmdb search %q: unexpected status %s: %s", query, resp.Status, statusMessage(resp.Body))
	}

	var body searchResponse
	if err := json.NewDecoder(resp.Body).Decode(&body); err != nil {
		return nil, fmt.Errorf("tmdb search %q: decode response: %w", query, err)
	}
